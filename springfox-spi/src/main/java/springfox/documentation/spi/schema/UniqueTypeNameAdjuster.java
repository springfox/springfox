...
public class UniqueTypeNameAdjuster {
  
  private static final Logger LOG = LoggerFactory.getLogger(UniqueTypeNameAdjuster.class);
  ...
  private void build(final ResolvedType type) {
    ...
    for(Integer modelId: modelIds) {
      if (links.containsKey(modelId)) {
        LOG.info("Skipping type with hash: {}, as link to another model {}", 
            modelId, 
            links.get(modelId));
        continue;
      }
