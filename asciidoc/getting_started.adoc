...
<14> Allows globally configuration of default path-/request-/headerparameters which are common for every rest operation of the api, but aren`t needed in method signature (for example authenticaton information).
<15> Sets up the security schemes used to protect the apis. Supported schemes are ApiKey, BasicAuth and OAuth
<16> Provides a way to globally set up security contexts for operation. The idea here is that we provide a way to
select operations to be protected by one of the specified security schemes.
<17> Here we use ApiKey as the security schema that is identified by the name `mykey`
<18> Selector for the paths this security context applies to.
<19> Here we same key defined in the security scheme `mykey`
<20> Optional swagger-ui security configuration for oauth and apiKey settings
<21> Optional swagger-ui ui configuration currently only supports the validation url
