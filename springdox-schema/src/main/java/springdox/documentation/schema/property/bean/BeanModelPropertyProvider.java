...
import static springdox.documentation.schema.Annotations.*;
import static springdox.documentation.schema.property.BeanPropertyDefinitions.*;
import static springdox.documentation.schema.property.bean.BeanModelProperty.*;
...
public class BeanModelPropertyProvider implements ModelPropertiesProvider {
  ...
  private ModelRef modelRef(ResolvedType type, ModelContext modelContext) {
    ...
    if (Maps.isMapType(type)) {
      String elementTypeName = typeNameExtractor.typeName(fromParent(modelContext, Maps.mapValueType(type)));
