...
class PathSelectorsSpec extends Specification {
  ...
  def "none predicate matches no RequestHandlers" () {
    expect:
      !PathSelectors.none().test("asdasdasdasd")
  }

  def "matches ant expressions"() {
    expect:
    shouldMatch == PathSelectors.ant(included).test(pathToMatch)
  ...
  def "matches regex expressions"() {
    given:
    expect:
    shouldMatch == PathSelectors.regex(included).test(patternConditions)
