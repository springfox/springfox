...
public class SpringIntegrationNameValueExpressionWrapper<T> implements NameValueExpression<T> {
  private org.springframework.web.servlet.mvc.condition.NameValueExpression<T> wrapped;

  private SpringIntegrationNameValueExpressionWrapper(
  ...
  public static <T> Set<NameValueExpression<T>> from(
    ...
    return springSet.stream()
        .map(SpringIntegrationNameValueExpressionWrapper::new)
