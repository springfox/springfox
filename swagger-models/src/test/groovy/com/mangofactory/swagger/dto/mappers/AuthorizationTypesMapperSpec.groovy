...
class AuthorizationTypesMapperSpec extends Specification {
  ...
  def "OAuth gets mapped correctly"() {
    ...
    and:
      ImplicitGrant mappedImplicitGrant = new ImplicitGrant(new com.mangofactory.swagger.dto.LoginEndpoint("uri:login"),
              "oauth-implicit")
  ...
  def "Authorization gets mapped correctly"() {
    ...
    when:
      com.mangofactory.swagger.dto.Authorization mapped = sut.toSwaggerAuthorization(built)
    then:
      mapped.type == built.type
      mapped.scopes.size() == 1
  ...
  def "Polymorphic authorization types are handled"() {
    given:
      List<com.mangofactory.service.model.AuthorizationType> listAuthType =
  ...
  def "AuthorizationTypesMapper handles unmapped authorization type"() {
    given:
      AuthorizationTypesMapper mapper = authMapper()
    when:
      mapper.toSwaggerAuthorizationType(new com.mangofactory.service.model.AuthorizationType("unknown") {
        @Override
        String getName() {
