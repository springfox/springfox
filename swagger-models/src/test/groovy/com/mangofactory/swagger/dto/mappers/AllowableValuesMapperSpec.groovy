...
class AllowableValuesMapperSpec extends Specification {
  def "Maps null range input to null output"() {
    ...
    when:
      com.mangofactory.swagger.dto.AllowableRangeValues dto = mapper.toSwaggerAllowableRangeValues(source)
    then:
      dto == source
  }
  ...
  def "Maps null list values input to null output"() {
    given:
      ...
      AllowableValuesMapper mapper = allowableValuesMapper()
    when:
      com.mangofactory.swagger.dto.AllowableListValues dto = mapper.toSwaggerAllowableListValues(source)
    then:
      dto == source
  }
