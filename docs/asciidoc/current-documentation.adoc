...
By default, Springfox infers server url from the request url of swagger endpoint (say /v3/api-docs).

This may not work if your application is behind a proxy with path prefix. In this case, you can specify a custom server url when building docket:
...
----
  ...
  public Docket swaggerSpringMvcPlugin() {
    return new Docket(DocumentationType.OAS_30)
            .servers(new ServerBuilder().name("relative").url("../").description("Relative server url").build());
  }
----

NOTE: Swagger client will resolve relative server url against swagger endpoint (say /v3/api-docs) instead of the Swagger UI url (say /swagger-ui/) you are visiting in Browser.
