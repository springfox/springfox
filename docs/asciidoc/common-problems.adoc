...
[NOTE]
Sometimes there are multiple `ObjectMapper`s in play and it may result in the customizations not working
...
A.  If you have recursively defined objects, I would try and see if providing an alternate type might work or perhaps
 even ignoring the offending classes for e.g. Order using the docket. ignoredParameterTypes(Order.class). This is
 usually found in Hibernate domain objects that have bi-dectional dependencies on other objects.
...
[[q19]]*Q. How are tags implemented in springfox?
 ...
 references to those Tags. The typical workflow is to registers tags in a docket and use the tag definitions on
