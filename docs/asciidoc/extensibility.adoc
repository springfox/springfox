...
1. Here is how we configure the `JacksonSerializerConvention`.
...
2. Below is an example for creating a rule that automatically provides a convention for configuring `Pageable` type.
