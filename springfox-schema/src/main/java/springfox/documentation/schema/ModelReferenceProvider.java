...
class ModelReferenceProvider implements Function<ResolvedType, ModelReference> {
  ...
  public ModelReferenceProvider(TypeNameExtractor typeNameExtractor, EnumTypeDeterminer enumTypeDeterminer,
      ModelContext parentContext, Map<String, String> knownNames) {
    this.typeNameExtractor = typeNameExtractor;
    this.enumTypeDeterminer = enumTypeDeterminer;
    this.parentContext = parentContext;
    this.knownNames = ImmutableMap.copyOf(knownNames);
  }
  ...
  private ModelReference modelReference(ResolvedType type) {
    ...
    return new ModelRef(typeName, type.getBriefDescription(), null, allowableValues(type),
  ...
  private Optional<ModelReference> mapReference(ResolvedType type) {
    if (isMapType(type)) {
      ...
      return Optional.<ModelReference>of(new ModelRef(typeName(type), apply(mapValueType), true));
    }
  ...
  private Optional<ModelReference> collectionReference(ResolvedType type) {
    if (isContainerType(type)) {
      ...
      return Optional.<ModelReference>of(new ModelRef(typeName(type), null, apply(collectionElementType),
          allowableValues(collectionElementType), null));
    }
    return Optional.absent();
  }

  private String typeName(ResolvedType type) {
    ModelContext context = fromParent(parentContext, type);
    return typeNameExtractor.typeName(context, knownNames);
  }
