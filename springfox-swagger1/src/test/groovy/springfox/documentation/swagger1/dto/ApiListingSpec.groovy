...
class ApiListingSpec extends InternalJsonSerializationSpec {
  ...
  def appendApiListing() {
    when:
      ApiListing apiListing = new ApiListing()
      apiListing.appendConsumes(Collections.singleton(MediaType.APPLICATION_JSON_VALUE))
      apiListing.appendProduces(Collections.singleton(MediaType.APPLICATION_JSON_VALUE))
      apiListing.appendProtocols(Collections.singleton("http"))
