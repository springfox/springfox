...
class AllowableValuesMapperSpec extends Specification {
  def "Maps null range input to null output"() {
    ...
    when:
      springdox.documentation.swagger.dto.AllowableRangeValues dto = mapper.toSwaggerAllowableRangeValues(source)
    then:
      dto == source
  }
  ...
  def "Maps null list values input to null output"() {
    given:
      ...
      AllowableValuesMapper mapper = allowableValuesMapper()
    when:
      springdox.documentation.swagger.dto.AllowableListValues dto = mapper.toSwaggerAllowableListValues(source)
    then:
      dto == source
  }
  ...
  def "Maps abstract values correctly correctly"() {
    ...
    then:
      rangeDto instanceof springdox.documentation.swagger.dto.AllowableRangeValues
      rangeDto.min == "0"
      rangeDto.max == "5"
      listDto instanceof springdox.documentation.swagger.dto.AllowableListValues
